...
print('This script computes CVPVI on unadjusted variables in GBM models.')
t0 <- Sys.time()
todays_date <- as.character(Sys.Date())
output_path = paste0("../output/SB-GBM_CVPVI_AllResidualized_", todays_date, ".rds")
...
pipeline_results = mclapply(X=ttSplits, mc.cores = 13, FUN=function(Z) {
  ...
  cvpvi = CVPVI(Model = gbm_mod, 
                Test = Z$test, 
                Permutations = Z$.PERMS, 
                FeatureNames = attr(gbm_mod$Terms, 'term.labels'),
                pred_func = predict.gbm_bestTree)
