# This version conducts residualization and drops IFC
#setup
print('ian j. douglas; contact: idouglas@utexas.edu')
print('This script runs the GBM pipeline, residualizes for all 3 confounds, and drops IFC subjects.')
t0 <- Sys.time()
todays_date <- as.character(Sys.Date())
output_path = paste0("../output/SB-GBM-final_AllResidualized_noIFC-Subjects_",
                     todays_date, 
                     ".rds")
print(paste0("Today's date: ", todays_date, ". Starting script today at ", t0, "."))
print(paste0('When complete, results will write to ', output_path))
nSplits <- 100
nNulls <- 100
set.seed(100)
