# This version conducts residualization, but not for ICV,  and drops IFC
#setup
print('ian j. douglas; contact: idouglas@utexas.edu')
print('This script will conduct the GBM pipeline.')
print('IFC participants are dropped, and ONLY ICV is controlled for in preprocessing')
t0 <- Sys.time()
todays_date <- as.character(Sys.Date())
output_path = paste0("../output/SB-GBM+onlyICV-Residualize+noIFC_",
                     todays_date, 
                     ".rds")
print(paste0("Today's date: ", todays_date, ". Starting script today at ", t0, "."))
print(paste0('Results will be written to ', output_path))
nSplits <- 100
nNulls <- 100
set.seed(100)
...
SB = cbind(SB %>% select(-starts_with(match = c("L.", 'R.'))),
           imap(SB %>% select(starts_with(match = c("L.", 'R.'))), ~Residualize_onlyICV(.y, SB)))
ELFK = cbind(ELFK %>% select(-starts_with(match = c("L.", 'R.'))),
             imap(ELFK %>% select(starts_with(match = c("L.", 'R.'))), ~Residualize_onlyICV(.y, ELFK)))
