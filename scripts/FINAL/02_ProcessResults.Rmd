...
ggplot(cvpvi.pltDat, aes(y = Variable)) +
  ...
  xlab('%-change in model performance (ROCAUC) associated with adding each variable to model') +
...
ecdf.pltDat %>%
  ...
  mutate(rank_id = dense_rank(pred_prob) / n()) %>% ungroup %>%
  ...
  mutate_at(vars(pipeline, preproc), ~case_when(
    ...
    grepl("AgeSexAdjusted$", .) ~ sub("[[:punct:]]", "", sub("AgeSexAdjusted", ": Age, Sex Controlled", .))
  )) %>%
  ...
  ggtitle("Predicted Probabilities of Early-Life Institutional Care for each Group, from each Model") +
  xlab('Probability') +
  ylab('Cumalitive Distribution')
