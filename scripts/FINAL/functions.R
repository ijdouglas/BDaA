...
# A function to do a permutation p-value for a model
permutation_test = function(.model, 
                            .new_X,
                            .new_y,
                            pred_func,
                            scoring_func,
                            ...
                            maximize = T) 
{
  # Some formatting checks for the arguments supplied and helpful warnings if so.
  try(TEST1 <- pred_func(.model, .new_X), silent = T)
  if (!exists('TEST1')) {
    stop('Please Note: pred_func MUST BE a function of `object` and `newdata`')
  } else {
    try(TEST2 <- scoring_func(TEST1, sample(.new_y)), silent = T)
    if (!exists('TEST2')) {
      stop('Please Note: scoring_func MUST BE a function of `y_pred` and `y_true`')
    }
  }
  # Tests passed, continue:
  require(parallel)
  if (parallel) {
    lambda = function(...) {mclapply(..., mc.cores = .cores)}
  } else if (isFALSE(parallel)) {lambda = function(...) lapply(...)}
  null_values = lambda(X = 1:R, FUN = function(r) {
    set.seed(r)
    .y_pred = pred_func(.model, .new_X)
    cross_val_score = scoring_func(.y_pred, sample(.new_y))
    cross_val_score
  }) %>% unlist
  observed = scoring_func(pred_func(.model, .new_X), .new_y)
  if (return_all) {return(null_values)}
  if (!return_all) {
    if (maximize) {
      p_val = (sum(null_values > observed) + 1) / (length(null_values) + 1)
      return(p_val)
    } else if (!maximize) {
      p_val = (sum(null_values < observed) + 1) / (length(null_values) + 1)
      return(p_val)
...
# A function to compute null accuracy scores, given just a scoring func, y_pred, and y_true
null_score = function(.y_pred, .y_true, .scoring_func, R = 1, pval = T, maximize = T)
{
  nulls = sapply(1:R, function(i) {
    set.seed(i)
    .scoring_func(.y_pred, sample(.y_true))
  })
  if (R > 1) {if (pval) {
    obs = .scoring_func(.y_pred, .y_true)
    if (maximize) {nulls = (sum(nulls > obs) + 1) / (length(nulls) + 1)}
    if (!maximize) {nulls = (sum(nulls < obs) + 1) / (length(nulls) + 1)}
  }}
  nulls
}
