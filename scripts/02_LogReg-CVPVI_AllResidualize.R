...
print('This script computes CVPVI on unadjusted variables in logistic regression models.')
t0 <- Sys.time()
todays_date <- as.character(Sys.Date())
output_path = paste0("../output/SB-LogReg-CVPVI_AllResidualized_", todays_date, ".rds")
...
## Produce permutations with which to randomize ELFK features to compute CVPVI
set.seed(1)
PERMS <- replicate(100, sample(nrow(ELFK)))

# Preproc:
## Replace all subcortical brain volume with residualized versions
SB = cbind(SB %>% select(-starts_with(match = c("L.", 'R.'))),
           imap(SB %>% select(starts_with(match = c("L.", 'R.'))), ~Residualize(.y, SB)))
ELFK = cbind(ELFK %>% select(-starts_with(match = c("L.", 'R.'))),
             imap(ELFK %>% select(starts_with(match = c("L.", 'R.'))), ~Residualize(.y, ELFK)))
...
pipeline_results = mclapply(X=ttSplits, mc.cores = 13, FUN=function(Z) {
  ...
  cvpvi = CVPVI(Model = glm_mod, 
                Test = Z$test, 
                Permutations = Z$.PERMS, 
                FeatureNames = attr(glm_mod$terms, 'term.labels'),
                pred_func = function(x, y) predict(x, y, type = 'response'))
